 ...
 *
 * Khách GỬI thông tin shop ở trang form.nguyenvantai.com (form Bot Mooly).
 * Flow (KISS, tái dùng hạ tầng leads của infor):
 *   1. Zod validate body (moolySubmitSchema)
 *   2. Normalize VN phone → canonical 0XXXXXXXXX
 *   3. Hash client IP (chống spam, không lưu IP gốc)
 *   4. RPC submit_lead - upsert lead theo SĐT, status → 'submitted', source 'mooly-form'
 *   5. Webhook fire-and-forget (Discord/n8n) nếu cấu hình
 *   6. Trả success + masked phone
 */

import { NextResponse, after } from 'next/server';
import { supabaseAdmin } from '@/lib/supabase/server-client';
import { moolySubmitSchema } from '@/lib/leads/mooly-schema';
...
export async function POST(request: Request) {
  ...
  const parsed = moolySubmitSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    console.error('[mooly] IP hash failed - submit continues without ip_hash');
  }
  ...
  if (rpcError) {
    console.error('[mooly] submit_lead RPC error', rpcError);
...
function jsonError(error: 'validation' | 'server', message: string, status: number, details?: unknown) {
...
async function sendWebhook(
  ...
  const lines = [
    `🆕 **Thông tin shop mới (Bot Mooly)**`,
    `👤 ${data.full_name} · ${data.phone}`,
    ...Object.entries(data.payload).map(([k, v]) => {
      const field = MOOLY_FIELD_BY_KEY[k];
      const label = MOOLY_FIELD_LABELS[k] ?? k;
      const text = (field ? formatLeadValue(field, v) : String(v ?? '')).trim();
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[mooly webhook] HTTP ${res.status}`, await res.text().catch(() => ''));
    }
  } catch (err) {
    console.error('[mooly webhook] dispatch failed', err);
  }
}
